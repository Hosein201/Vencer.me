! function (e) {
    ...
    t.prototype.onDrop = function (t, n) {
        ...
        var o = this,
            i = e("<form></form>");
        i.append("<label class='float-left'>تغییر نام رویداد</label>"), i.append("<div class='input-group'><input class='form-control' type=text value='" + t.title + "' /><span class='input-group-btn'><button type='submit' class='btn btn-success waves-effect waves-light'><i class='fa fa-check'></i> ذخیره</button></span></div>"), o.$modal.modal({
            backdrop: "static"
        ...
        var i = e("<form></form>");
        i.append("<div class='row'></div>"), i.find(".row").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>نام رویداد</label><input class='form-control' placeholder='نام رویداد را وارد کنید' type='text' name='title'/></div></div>").append("<div class='col-md-6'><div class='form-group'><label class='control-label'>رنگ رویداد</label><select class='form-control' name='category'></select></div></div>").find("select[name='category']").append("<option value='bg-danger'>خطر</option>").append("<option value='bg-success'>موفق</option>").append("<option value='bg-dark'>تیره</option>").append("<option value='bg-primary'>اولیه</option>").append("<option value='bg-pink'>صورتی</option>").append("<option value='bg-info'>اطلاع</option>").append("<option value='bg-warning'>هشدار</option></div></div>"), o.$modal.find(".delete-event").hide().end().find(".save-event").show().end().find(".modal-body").empty().prepend(i).end().find(".save-event").unbind("click").on("click", function () {
            i.submit()
        }), o.$modal.find("form").on("submit", function () {
